...
class MMoE(nn.Module):
    ...
    def __init__(self, input_size, num_experts, experts_out, experts_hidden, towers_hidden, output_size, tasks):
        ...
        self.experts = nn.ModuleList(
            [Expert(self.input_size, self.experts_hidden, self.experts_out) for _ in range(self.num_experts)])
        self.gates = nn.ParameterList(
            [nn.Parameter(torch.randn(self.input_size, self.num_experts), requires_grad=True) for _ in
             range(self.tasks)])
        self.towers = nn.ModuleList(
            [Tower(self.experts_out, self.towers_hidden, self.output_size) for _ in range(self.tasks)])
